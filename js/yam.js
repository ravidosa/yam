        ...
        for (const manga of reading) {
            if (manga.publisher == "VIZ Media") {
                try {
                    ...
                    let response = await fetch(`https://corsproxy.io/?https%3A%2F%2Fwww.viz.com%2F${slugify(manga.title.en)}`);
                ...
                <div id="select">
                    <form id="form"  key={this.state.selected}>
                        ...
                        <select id="select-publisher" name="publisher">
                            ...
                            {["DENPA", "Kodansha", "Medibang", "Seven Seas", "TOKYOPOP", "Udon Entertainment", "VIZ Media", "Yen Press", "Square Enix", "One Peace Books", "Vertical", "SuBLime"].sort().map((publisher, index) => {
                                return (
